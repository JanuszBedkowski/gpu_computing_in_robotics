...
{
	std::cout << "Lesson 11 - data registration point to projection Iterative Closest Point" << std::endl;
...
bool initGL(int *argc, char **argv)
{
    ...
    glutCreateWindow("Lesson 11 - data registration point to projection Iterative Closest Point");
    glutDisplayFunc(display);
    glutKeyboardFunc(keyboard);
    glutMotionFunc(motion);
    glutReshapeFunc(reshape);
...
void keyboard(unsigned char key, int /*x*/, int /*y*/)
{
    switch (key)
    {
		...
		case 'r' :
		{
			...
			Eigen::Affine3d mICP = Eigen::Affine3d::Identity();

			if(!cudaWrapper.dataRegistrationPoitnToProjectionICP(
					first_point_cloud,
					second_point_cloud,
					normal_vectors_search_radius,
					projections_search_radius,
					...
					mICP))
			{
				std::cout << "cudaWrapper.dataRegistrationPoitnToProjectionICP NOT SUCCESFULL" << std::endl;
			}else
			{
			...
			computation_time=(double)( clock () - begin_time ) /  CLOCKS_PER_SEC;
			std::cout << "cudaWrapper.dataRegistrationPoitnToProjectionICP computation_time: " << computation_time << std::endl;
...
{
	std::cout << "----------------------" << std::endl;
	std::cout << "press 'r': data registration with ICP point to projection - 1 iteration" << std::endl;
