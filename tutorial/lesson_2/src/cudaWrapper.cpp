...
bool CCudaWrapper::removeNoiseNaive(pcl::PointCloud<pcl::PointXYZ> &point_cloud, float resolution, int number_of_points_in_bucket_threshold)
{
	cudaError_t err = ::cudaSuccess;
	err = cudaSetDevice(0);
	...
	err = cudaRemoveNoiseNaive(threads, d_markers, d_point_cloud, d_hashTable, d_buckets, rgd_params, point_cloud.points.size(), number_of_points_in_bucket_threshold);
			if(err != ::cudaSuccess)return false;
