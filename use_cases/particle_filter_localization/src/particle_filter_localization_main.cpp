...
float nearest_neighbor_radius = 1.0f;
...
CParticleFilter particle_filter;
...
{
	std::cout << "use case: particle filter localization" << std::endl;
	...
	if(argc < 3)
	{
		...
		std::cout << "-nnr: nearest_neighbor_radius  default: " << nearest_neighbor_radius << std::endl;
	...
	}else
	{
		...
		pcl::console::parse_argument (argc, argv, "-nnr", nearest_neighbor_radius);
		std::cout << "nearest_neighbor_radius: " << nearest_neighbor_radius << std::endl;
...
bool initGL(int *argc, char **argv)
{
    ...
    glutCreateWindow("Particle Filter Localization");
    glutDisplayFunc(display);
    glutKeyboardFunc(keyboard);
    glutMotionFunc(motion);
    glutReshapeFunc(reshape);
